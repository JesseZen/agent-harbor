...
func releaseAssetPath(t *testing.T, relative string) string {
	t.Helper()
	path := filepath.Join(repoRoot(t), relative)
	if _, err := os.Stat(path); err != nil {
		if os.IsNotExist(err) {
			t.Skipf("legacy Agent Deck release asset %s is not included in this distribution", relative)
		}
		t.Fatalf("stat release asset %s: %v", path, err)
	}
	return path
}
...
func loadManifest(t *testing.T) (*manifestDoc, string) {
	t.Helper()
	root := repoRoot(t)
	path := releaseAssetPath(t, filepath.Join(".claude", "release-tests.yaml"))
