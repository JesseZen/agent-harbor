...
func TestResponsiveGoldens(t *testing.T) {
	...
	for _, size := range sizes {
		t.Run(size.name, func(t *testing.T) {
			...
			if !strings.Contains(view, resourceview.TokenBorder) && !strings.Contains(view, "Routes") {
				t.Fatalf("%s missing K9s chrome:\n%s", size.name, view)
			}

			path := filepath.Join("testdata", "golden", "routes-"+size.name+".ansi")
			if *updateGoldens {
...
func TestSimpleTrafficRulesResponsiveGoldens(t *testing.T) {
	page := New(simpleTrafficDraft(t))
	...
	for _, size := range sizes {
		t.Run(size.name, func(t *testing.T) {
			...
			view := page.View()
			plain := ansi.Strip(view)
			for _, column := range []string{"CLIENT", "PRIMARY"} {
			...
			path := filepath.Join("testdata", "golden", "traffic-rules-"+size.name+".ansi")
			if *updateGoldens {
				if err := os.WriteFile(path, []byte(view), 0o644); err != nil {
					t.Fatalf("write golden: %v", err)
				}
...
func TestTrafficRuleFormResponsiveGoldens(t *testing.T) {
	page := New(simpleTrafficDraft(t))
	page.BeginCreate()
	for _, size := range []struct {
		...
		t.Run(size.name, func(t *testing.T) {
			...
			path := filepath.Join("testdata", "golden", "traffic-rule-form-"+size.name+".ansi")
			if *updateGoldens {
				if err := os.WriteFile(path, []byte(view), 0o644); err != nil {
					t.Fatal(err)
				}
