...
func TestCanRestartCursor(t *testing.T) {
	...
	binDir := t.TempDir()
	fakeCursor := filepath.Join(binDir, "cursor")
	if err := os.WriteFile(fakeCursor, []byte("#!/bin/sh\nsleep 60\n"), 0o755); err != nil {
		t.Fatalf("write fake cursor: %v", err)
	}
	...
	if inst.tmuxSession == nil || !waitForTmuxSession(inst.tmuxSession.Name, time.Second) {
