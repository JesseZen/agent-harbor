...
func readCapturedClaudeArgv(t *testing.T, logPath string, timeout time.Duration) []string {
	t.Helper()
	return readCapturedClaudeArgvUntil(t, logPath, timeout, func(argv []string) bool {
		return len(argv) > 0
	})
}

func readCapturedClaudeArgvUntil(
	t *testing.T,
	logPath string,
	...
	t.Fatalf("readCapturedClaudeArgv: expected argv was not captured in %s at %s; last argv=%v", timeout, logPath, last)
