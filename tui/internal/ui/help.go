...
// NewHelpOverlayWithContent creates the standard help overlay with caller-supplied content.
func NewHelpOverlayWithContent(content HelpContent) *HelpOverlay {
	cloned := HelpContent{Footer: content.Footer, Sections: make([]HelpSection, len(content.Sections))}
	for sectionIndex, section := range content.Sections {
		cloned.Sections[sectionIndex] = HelpSection{
			Title: section.Title,
			Items: append([]HelpItem(nil), section.Items...),
		}
	}
	return &HelpOverlay{
...
func (h *HelpOverlay) View() string {
	...
	footer := "Agent Deck v" + Version
	if h.content != nil {
		sections = make([]helpRenderSection, 0, len(h.content.Sections))
		for _, section := range h.content.Sections {
			renderSection := helpRenderSection{
				title: section.Title,
				items: make([][2]string, 0, len(section.Items)),
			}
			for _, item := range section.Items {
				renderSection.items = append(renderSection.items, [2]string{item.Key, item.Description})
			}
	...
	lines = append(lines, versionStyle.Render(footer))
