...
func TestHelpOverlayRendersCustomContent(t *testing.T) {
	overlay := NewHelpOverlayWithContent(HelpContent{
		Sections: []HelpSection{
			{Title: "ROUTES", Items: []HelpItem{{Key: "j / k", Description: "Select route"}}},
			{Title: "GLOBAL", Items: []HelpItem{{Key: ":", Description: "Spotlight"}}},
		},
	...
	view := overlay.View()
	for _, expected := range []string{"KEYBOARD SHORTCUTS", "ROUTES", "Select route", "GLOBAL", "Spotlight", "Agent Harbor TUI"} {
		if !strings.Contains(view, expected) {
			t.Fatalf("custom help omitted %q:\n%s", expected, view)
		}
	}
	if strings.Index(view, "ROUTES") > strings.Index(view, "GLOBAL") {
		t.Fatal("tab-specific help should precede global help")
	}

	overlay.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{':'}})
	if overlay.IsVisible() {
		t.Fatal("custom help did not preserve Agent Deck's ordinary-key dismissal")
	}
}
