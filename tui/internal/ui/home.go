...
type Home struct {
	...
	backend backend.SessionLoader
	// backendGeneration and backendSessions retain only public Core version
	// metadata needed to issue typed actions from the existing Agent Deck UI.
	backendGeneration int64
	backendSessions   map[string]backend.Session
	backendProfiles   map[string]backend.ClientProfile
	backendRoutes     map[string]backend.Route
	...
	previewErrors     map[string]string    // previewKey -> terminal fetch error state
...
// applyUIConfig caches display/footer settings used by Home chrome (help bar, preview).
func (h *Home) applyUIConfig(profile string) {
	if cfg, _ := session.LoadUserConfig(); cfg != nil {
		...
		h.costLineTemplate, h.costLineHideWhenZero = session.ResolveCostLineTemplate(cfg, profile)
	...
	h.costLineTemplate, h.costLineHideWhenZero = session.ResolveCostLineTemplate(nil, profile)
...
func (h *Home) quickApprove(inst *session.Instance, windowIndex int) {
	...
	if h.rejectCoreLocalAction("quick approve") {
		return
	}
...
func (h *Home) openPromptInput(inst *session.Instance) {
	...
	if h.rejectCoreLocalAction("prompt input") {
		return
	}
...
type statusUpdateMsg struct {
	...
	err               error  // Core attach preparation or helper failure
...
func NewHomeWithProfileAndMode(profile string) *Home {
	return newHome(profile, nil)
}

// NewHomeWithBackend creates the upstream Agent Deck Home with an external
// runtime backend. Presentation and ephemeral UI state remain owned by Home;
// storage, tmux, hooks, processes, and persistence remain owned by the backend.
func NewHomeWithBackend(source backend.SessionLoader) *Home {
	return newHome("", source)
}

// startHomeRuntimeServices remains enabled in production. TestMain disables it
// for package tests whose Home models are short-lived values: starting real
// tmux, watcher, and status workers for every constructor otherwise leaks
// goroutines and subprocesses across the full test suite.
var startHomeRuntimeServices = true

func newHome(profile string, source backend.SessionLoader) *Home {
	...
	h := &Home{
		...
		backendSessions:           make(map[string]backend.Session),
		backendProfiles:           make(map[string]backend.ClientProfile),
		backendRoutes:             make(map[string]backend.Route),
	...
	// Hotkeys + footer/display chrome are required for both storage-backed and
	// backend-backed Homes. NewHomeWithBackend used to return early with an empty
	// hotkey map, so the Sessions help bar only showed hardcoded keys (Tab/Enter).
	h.reloadHotkeysFromConfig()
	h.applyUIConfig(actualProfile)
...
func (h *Home) loadSessions() tea.Msg {
	if h.backend != nil {
		snapshot, err := h.backend.LoadSessions(h.ctx)
		return loadSessionsMsg{instances: coreBackendInstances(snapshot), backendSnapshot: &snapshot, err: err}
	}
	if h.storage == nil {
...
func (h *Home) invalidatePreviewCache(sessionID string) {
	...
	delete(h.previewErrors, sessionID)
...
func (h *Home) fetchPreview(inst *session.Instance, key string, windowIndex int) tea.Cmd {
	...
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			ref, refErr := h.backendSessionRef(inst.ID)
			return func() tea.Msg {
				if refErr != nil {
					return previewFetchedMsg{previewKey: key, err: refErr}
				}
				preview, err := source.Preview(h.ctx, ref)
				return previewFetchedMsg{previewKey: key, content: strings.Join(preview.Lines, "\n"), err: err}
			}
		}
		return func() tea.Msg {
			return previewFetchedMsg{previewKey: key, err: backend.ErrUnsupported}
		}
	}
	return func() tea.Msg {
		var content string
...
func (h *Home) updateInner(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case loadSessionsMsg:
		...
		if msg.err == nil && msg.backendSnapshot != nil {
			h.backendGeneration = msg.backendSnapshot.Generation
			h.backendSessions = make(map[string]backend.Session, len(msg.backendSnapshot.Sessions))
			for _, source := range msg.backendSnapshot.Sessions {
				h.backendSessions[source.ID] = source
			}
			h.backendProfiles = make(map[string]backend.ClientProfile, len(msg.backendSnapshot.ClientProfiles))
			for _, profile := range msg.backendSnapshot.ClientProfiles {
				h.backendProfiles[profile.ID] = profile
			}
			h.backendRoutes = make(map[string]backend.Route, len(msg.backendSnapshot.Routes))
			for _, route := range msg.backendSnapshot.Routes {
				h.backendRoutes[route.ID] = route
			}
		}
		if h.backend == nil {
			h.reloadHotkeysFromConfig()
		...
		if h.backend == nil && h.pendingHooksPrompt && !h.setupWizard.IsVisible() {
		...
		if h.backend == nil && h.feedbackDialog != nil && !h.feedbackDialog.IsVisible() {
	...
	case sessionCreatedMsg:
		...
		if msg.err != nil {
			...
			if msg.backendSession != nil {
				h.backendSessions[msg.backendSession.ID] = *msg.backendSession
			}
			h.instancesMu.Lock()
			h.instances = append(h.instances, msg.instance)
	...
	case sessionDeletedMsg:
		...
		if msg.killErr != nil && h.backend != nil {
			h.setError(fmt.Errorf("failed to end Core session: %w", msg.killErr))
			return h, nil
		}
		...
		if deletedInstance != nil && h.backend == nil {
		...
		delete(h.backendSessions, msg.deletedID)
	...
	case sessionClosedMsg:
		...
		if h.backend != nil {
			delete(h.backendSessions, msg.sessionID)
	...
	case sessionUnarchivedMsg:
		if msg.err != nil {
			h.setError(fmt.Errorf("failed to unarchive: %w", msg.err))
			return h, nil
		}
		h.rebuildFlatItems()
		h.saveInstances()
	...
	case sessionRestartedMsg:
		if msg.err != nil {
			...
			if msg.backendSession != nil {
				source := h.decorateBackendSession(*msg.backendSession)
				h.backendSessions[msg.sessionID] = source
				if inst := h.getInstanceByID(msg.sessionID); inst != nil {
					inst.Title = source.Title
					inst.ProjectPath = source.ProjectPath
					inst.GroupPath = source.GroupPath
					inst.Tool = source.Tool
					inst.Status = coreBackendDisplayStatus(source)
					inst.CreatedAt = source.CreatedAt
					inst.LastAccessedAt = source.LastAccessedAt
				}
				h.invalidatePreviewCache(msg.sessionID)
				delete(h.resumingSessions, msg.sessionID)
				return h, nil
	...
	case statusUpdateMsg:
		...
		if h.backend != nil {
			if msg.err != nil {
				h.setError(fmt.Errorf("failed to attach session: %w", msg.err))
			}
			h.beginAttachReturnGrace(time.Now())
			return h, tea.Batch(
				tea.EnableMouseCellMotion,
	...
	case previewFetchedMsg:
		...
		if msg.err == nil {
			...
			delete(h.previewErrors, msg.previewKey)
		} else {
			h.previewErrors[msg.previewKey] = fmt.Sprintf("Preview unavailable: %v; retrying automatically", msg.err)
		}
		h.previewCacheMu.Unlock()
		return h, nil
	...
	case msg.String() == "enter" || submitNow:
		...
		if h.backend != nil {
			...
			return h, h.createSessionInGroupWithWorktreeAndOptions(
				name, path, command, groupPath, "", "", "", false, false,
...
// CanOpenGlobalCommandMenu reports whether Home is in ordinary browsing mode
// and therefore permits the outer application to claim a global shortcut.
// Home remains the sole owner of its insert, dialog, search, editor, picker,
// and modal state; callers only receive this read-only permission boundary.
func (h *Home) CanOpenGlobalCommandMenu() bool {
	if h.insertMode || h.hasModalVisible() || h.showCostDashboard {
		return false
	}
	return h.feedbackDialog == nil || !h.feedbackDialog.IsVisible()
}
	...
	case tea.MouseButtonLeft:
		...
		if isDoubleClick {
			...
			if item.Type == session.ItemTypeSession && item.Session != nil {
				...
				if h.backend != nil {
					return h, h.activateCoreSession(item.Session)
				}
...
func (h *Home) handleMainKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "shift+enter":
		if h.rejectCoreLocalAction("open in a separate terminal") {
			return h, nil
		}
	...
	case "shift+up", "ctrl+up", "+", "K":
		if h.rejectCoreLocalAction("local session ordering") {
			return h, nil
		}
		// Move item up
		if h.cursor < len(h.flatItems) {
	...
	case "shift+left":
		if h.rejectCoreLocalAction("local session nesting") {
			return h, nil
		}
	...
	case "p":
		if h.rejectCoreLocalAction("local workspace editing") {
			return h, nil
		}
	...
	case "P", "shift+p":
		if h.rejectCoreLocalAction("local session settings") {
			return h, nil
		}
	...
	case "m":
		if h.rejectCoreLocalAction("local MCP management") {
			return h, nil
		}
	...
	case "L":
		if h.rejectCoreLocalAction("local plugin management") {
			return h, nil
		}
	...
	case "f":
		if h.rejectCoreLocalAction("local session fork") {
			return h, nil
		}
	...
	case "s":
		if h.rejectCoreLocalAction("local skill management") {
			return h, nil
		}
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
	...
	case "M", "shift+m":
		if h.rejectCoreLocalAction("local route mutation") {
			return h, nil
		}
	...
	case "b":
		if h.rejectCoreLocalAction("local worktree setup") {
			return h, nil
		}
	...
	case "W", "shift+w":
		if h.rejectCoreLocalAction("local worktree finish") {
			return h, nil
		}
	...
	case "g":
		...
		if h.rejectCoreLocalAction("local group creation") {
			return h, nil
		}
	...
	case "r":
		if h.rejectCoreLocalAction("local rename") {
			return h, nil
		}
	...
	case "S":
		if h.rejectCoreLocalAction("local settings") {
			return h, nil
		}
		// Open settings panel
		h.settingsPanel.Show()
	...
	case "w":
		if h.rejectCoreLocalAction("local watcher management") {
			return h, nil
		}
		// Open watcher panel
		h.refreshWatcherPanel()
	...
	case "E":
		if h.rejectCoreLocalAction("local container shell") {
			return h, nil
		}
	...
	case "N":
		if h.backend != nil {
			return h, h.quickCreateCoreSession()
		}
	...
	case "z":
		if h.rejectCoreLocalAction("local zoxide process") {
			return h, nil
		}
	...
	case "d":
		...
		if h.cursor < len(h.flatItems) {
			...
			} else if item.Type == session.ItemTypeGroup && h.backend != nil {
				h.rejectCoreLocalAction("local group deletion")
	...
	case "i":
		if h.rejectCoreLocalAction("session import") {
			return h, nil
		}
		return h, h.importSessions

	case "I":
		if h.rejectCoreLocalAction("insert mode") {
			return h, nil
		}
	...
	case "u":
		if h.rejectCoreLocalAction("mark unread") {
			return h, nil
		}
	...
	case "y":
		if h.rejectCoreLocalAction("local tool options") {
			return h, nil
		}
	...
	case "c":
		if h.rejectCoreLocalAction("copy through local clipboard process") {
			return h, nil
		}
	...
	case "x":
		if h.rejectCoreLocalAction("send output to another session") {
			return h, nil
		}
	...
	case "e":
		if h.rejectCoreLocalAction("session notes") {
			return h, nil
		}
	...
	case "ctrl+g":
		if h.rejectCoreLocalAction("local model selection") {
			return h, nil
		}
	...
	case "ctrl+z":
		if h.rejectCoreLocalAction("undo local deletion") {
			return h, nil
		}
	...
	case "ctrl+r":
		...
		if h.backend != nil {
			return h, func() tea.Msg {
				message := h.loadSessions()
				if loaded, ok := message.(loadSessionsMsg); ok {
	...
	case "ctrl+e":
		if h.rejectCoreLocalAction("local feedback and telemetry") {
			return h, nil
		}
...
func (h *Home) createSessionInGroupWithWorktreeAndOptions(
	...
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			return h.createCoreSession(source, name, path, command, groupPath, tempID)
		}
		return func() tea.Msg { return sessionCreatedMsg{err: backend.ErrUnsupported, tempID: tempID} }
	}
	return func() tea.Msg {
...
func (h *Home) deleteSession(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			return h.endCoreSession(source, inst, backend.EndForce)
		}
		id := inst.ID
		return func() tea.Msg { return sessionDeletedMsg{deletedID: id, killErr: backend.ErrUnsupported} }
	}
	id := inst.ID
...
func (h *Home) closeSession(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			return h.endCoreSession(source, inst, backend.EndGraceful)
		}
		id := inst.ID
		return func() tea.Msg { return sessionClosedMsg{sessionID: id, killErr: backend.ErrUnsupported} }
	}
	id := inst.ID
	return func() tea.Msg {
		killErr := inst.Kill()
...
func (h *Home) archiveSession(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		id := inst.ID
		return func() tea.Msg { return sessionArchivedMsg{sessionID: id, killErr: backend.ErrUnsupported} }
	}
...
func (h *Home) unarchiveSession(inst *session.Instance) tea.Cmd {
	id := inst.ID
	if h.backend != nil {
		return func() tea.Msg { return sessionUnarchivedMsg{sessionID: id, err: backend.ErrUnsupported} }
	}
	return func() tea.Msg {
...
func (h *Home) bulkRemoveErrored() tea.Cmd {
	if h.backend != nil {
		...
		if !ok {
			return func() tea.Msg { return sessionDeletedMsg{killErr: backend.ErrUnsupported} }
		}
		h.instancesMu.RLock()
		...
		commands := make([]tea.Cmd, 0, len(instances))
		for _, inst := range instances {
			commands = append(commands, h.endCoreSession(source, inst, backend.EndForce))
		}
...
func (h *Home) restartSession(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			return h.restartCoreSession(source, inst, false)
		}
		id := inst.ID
		return func() tea.Msg { return sessionRestartedMsg{sessionID: id, err: backend.ErrUnsupported} }
	}
	id := inst.ID
	mcpUILog.Debug(
...
func (h *Home) restartSessionFresh(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		if source, ok := h.runtimeBackend(); ok {
			return h.restartCoreSession(source, inst, true)
		}
		id := inst.ID
		return func() tea.Msg { return sessionRestartedMsg{sessionID: id, err: backend.ErrUnsupported, fresh: true} }
	}
	id := inst.ID
	mcpUILog.Debug(
...
func (h *Home) attachSession(inst *session.Instance) tea.Cmd {
	if h.backend != nil {
		source, ok := h.runtimeBackend()
		if !ok {
			return func() tea.Msg { return statusUpdateMsg{attachedSessionID: inst.ID, err: backend.ErrUnsupported} }
		}
		ref, err := h.backendSessionRef(inst.ID)
		if err != nil {
			return func() tea.Msg { return statusUpdateMsg{attachedSessionID: inst.ID, err: err} }
		}
		h.isAttaching.Store(true)
		return tea.Exec(&coreAttachExecCmd{ctx: h.ctx, source: source, ref: ref}, func(err error) tea.Msg {
			h.isAttaching.Store(false)
...
func (h *Home) renderPreviewPane(width, height int) string {
	...
	if source, ok := h.backendSessions[selected.ID]; ok {
		if source.NativeActivity != "" {
			activity := string(source.NativeActivity)
			if source.ActivitySource != "" {
				activity += " via " + string(source.ActivitySource)
			}
			if !source.ActivityObservedAt.IsZero() {
				activity += " · " + formatRelativeTime(source.ActivityObservedAt)
			}
			b.WriteString(infoStyle.Render("Activity: " + activity))
			b.WriteString("\n")
		}
		if source.HookHealth != "" {
			hook := string(source.HookHealth)
			if !source.HookHealthObservedAt.IsZero() {
				hook += " · " + formatRelativeTime(source.HookHealthObservedAt)
			}
			b.WriteString(infoStyle.Render("Hook: " + hook))
			b.WriteString("\n")
		}
		if source.NativeProvider != "" || source.NativeSessionID != "" {
			native := string(source.NativeProvider)
			if native == "" {
				native = "unbound"
			}
			if source.NativeSessionID != "" {
				native += " / " + source.NativeSessionID
			}
			b.WriteString(infoStyle.Render("Native: " + native))
			b.WriteString("\n")
		}
	}
	...
	previewError := h.previewErrors[pvKey]
	h.previewCacheMu.RUnlock()
	...
	if isSetupRunning {
		...
		errorStyle := lipgloss.NewStyle().
			Foreground(ColorYellow).
			Italic(true)
		b.WriteString(errorStyle.Render(previewError))
	} else if !hasCached {
