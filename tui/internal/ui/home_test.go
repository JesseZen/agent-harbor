...
func TestCorePreviewErrorExitsLoadingState(t *testing.T) {
	instance := &session.Instance{
		ID: "ses-preview-error", Title: "Preview error", Tool: "codex",
		Status: session.StatusRunning, ProjectPath: "/workspace/error",
	}
	home := newTestHomeWithItems(100, 30, []session.Item{{
		Type: session.ItemTypeSession, Session: instance,
	}})
	home.cursor = 0

	model, _ := home.Update(previewFetchedMsg{
		previewKey: instance.ID,
		err:        fmt.Errorf("backend unavailable"),
	})
	home = model.(*Home)
	rendered := home.renderPreviewPane(80, 20)

	if !strings.Contains(rendered, "Preview unavailable") {
		t.Fatalf("preview error did not reach a terminal UI state: %q", rendered)
	}
	if !strings.Contains(rendered, "backend unavailable") {
		t.Fatalf("preview error detail was hidden: %q", rendered)
	}
	if strings.Contains(rendered, "Loading preview...") {
		t.Fatalf("preview error remained in loading state: %q", rendered)
	}
}
