...
type NewDialog struct {
	...
	coreMode     bool
	coreProfiles []backend.ClientProfile
}
...
func (d *NewDialog) ShowInGroup(groupPath, groupName, defaultPath string, conductors []*session.Instance, suggestedParentID string) {
	if d.coreMode {
		selectedLauncher := ""
		if d.commandCursor >= 0 && d.commandCursor < len(d.coreProfiles) {
			selectedLauncher = d.coreProfiles[d.commandCursor].Launcher
		}
		d.coreMode = false
		d.coreProfiles = nil
		d.presetCommands = buildPresetCommands()
		d.commandCursor = 0
		for index, command := range d.presetCommands {
			if command == selectedLauncher {
				d.commandCursor = index
				break
			}
		}
	}
	d.showInGroup(groupPath, groupName, defaultPath, conductors, suggestedParentID, true)
}

// ShowCoreInGroup shows only values accepted by Core session creation.
func (d *NewDialog) ShowCoreInGroup(groupPath, groupName, defaultPath string, profiles []backend.ClientProfile, selectedProfile string) {
	d.coreMode = true
	d.coreProfiles = append([]backend.ClientProfile(nil), profiles...)
	d.presetCommands = make([]string, 0, len(profiles))
	d.commandCursor = 0
	for index, profile := range profiles {
		d.presetCommands = append(d.presetCommands, profile.ID)
		if profile.ID == selectedProfile || profile.Launcher == selectedProfile {
			d.commandCursor = index
		}
	}
	d.showInGroup(groupPath, groupName, defaultPath, nil, "", false)
}

func (d *NewDialog) showInGroup(groupPath, groupName, defaultPath string, conductors []*session.Instance, suggestedParentID string, loadRuntimeDefaults bool) {
	if groupPath == "" {
		groupPath = "default"
		groupName = "default"
	}
	...
	if userConfig, err := session.LoadUserConfig(); loadRuntimeDefaults && err == nil && userConfig != nil {
...
func (d *NewDialog) SetDefaultTool(tool string) {
	...
	if d.coreMode {
		for index, profile := range d.coreProfiles {
			if profile.ID == tool || profile.Launcher == tool {
				d.commandCursor = index
				d.updateToolOptions()
				return
			}
...
func (d *NewDialog) rebuildFocusTargets() {
	if d.coreMode {
		d.focusTargets = []focusTarget{focusName, focusCommand, focusPath}
...
func (d *NewDialog) updateFocus() {
	...
	case focusCommand:
		if !d.coreMode && d.commandCursor == 0 { // shell.
...
func (d *NewDialog) isTextInputFocused() bool {
	...
	case focusName, focusPath, focusModel, focusBranch:
		return true
	case focusCommand:
		return !d.coreMode && d.commandCursor == 0 // custom command input
	case focusMultiRepo:
...
func (d *NewDialog) Update(msg tea.Msg) (*NewDialog, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "w":
			if !d.coreMode && cur == focusCommand && !d.isTextInputFocused() {
				d.ToggleWorktree()
				d.rebuildFocusTargets()
				if d.worktreeEnabled {
	...
	case focusCommand:
		if !d.coreMode && d.commandCursor == 0 {
...
func (d *NewDialog) renderCommandSection(content *strings.Builder, cur focusTarget) {
	...
	if d.coreMode {
		commandLabel = "Traffic Rule"
	}
	if cur == focusCommand {
		content.WriteString(activeLabelStyle.Render("▶ " + commandLabel + ":"))
	} else {
		content.WriteString(labelStyle.Render("  " + commandLabel + ":"))
	}
	content.WriteString("\n  ")
	...
	for i, cmd := range d.presetCommands {
		displayName := cmd
		if d.coreMode && i < len(d.coreProfiles) {
			displayName = d.coreProfiles[i].Name
			if displayName == "" {
				displayName = d.coreProfiles[i].Launcher
			}
		} else if displayName == "" {
			displayName = "shell"
		} else {
	...
	if !d.coreMode && session.ToolFilterFallbackActive() {
...
func (d *NewDialog) coreRouteLabel() string {
	if d.coreMode && d.commandCursor >= 0 && d.commandCursor < len(d.coreProfiles) {
		profile := d.coreProfiles[d.commandCursor]
		if profile.DefaultRouteName != "" {
			return profile.DefaultRouteName
		}
		if profile.DefaultRouteID != "" {
			return profile.DefaultRouteID
		}
	}
	return d.parentGroupName
}
...
func (d *NewDialog) View() string {
	...
	// Title with parent group info. In Core mode the selected Traffic Rule owns
	// the route, so exposing a second Route row would duplicate an internal
	// implementation detail.
	content.WriteString(titleStyle.Render("New Session"))
	content.WriteString("\n")
	if !d.coreMode {
		groupInfoStyle := lipgloss.NewStyle().Foreground(ColorPurple)
		content.WriteString(groupInfoStyle.Render("  in group: " + d.coreRouteLabel()))
		content.WriteString("\n")
	}
	...
	if cur == focusPath {
		...
		if d.coreMode {
			helpText = "←→ traffic rule │ Tab next │ ^S create │ Esc cancel"
		} else {
			selectedCmd := d.GetSelectedCommand()
	...
	// Wrap in dialog box
	dialog := formui.DialogFrame(content.String(), dialogWidth, 2, 4)
