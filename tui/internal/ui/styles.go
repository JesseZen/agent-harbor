...
// Accepts named registry ids plus legacy aliases "dark"/"light".
// Must be called before any UI rendering.
func InitTheme(theme string) {
	themeMu.Lock()
	defer themeMu.Unlock()
	name := resolveThemeName(theme)
	palette, ok := themePalettes[name]
	if !ok {
		name = "tokyonight"
		palette = themePalettes[name]
	}
	currentThemeName = name
	applyPalette(palette)
	initStyles()
	syncResourceviewTheme()
}

// syncResourceviewTheme pushes the active AH palette into k9s table tokens
// so :theme updates headers, selection, column rules, and kind strips.
// Hierarchy: selected row (Accent-tinted) > column header band (Surface) > idle rules.
func syncResourceviewTheme() {
	resourceview.ApplyTheme(resourceview.ThemeTokens{
		OnAccent:     "#000000",
		Border:       string(ColorBorder),
		Header:       string(ColorCyan),
		HeaderBg:     string(ColorSurface),
		Text:         string(ColorText),
		Healthy:      string(ColorGreen),
		Warning:      string(ColorYellow),
		Error:        string(ColorRed),
		Selected:     blendHex(ColorSurface, ColorAccent, 0.42),
		Accent:       string(ColorAccent),
		RuleIdle:     string(ColorComment),
		RuleSelected: string(ColorAccent),
	})
}

// blendHex lerps two #RRGGBB colors; t=0 yields a, t=1 yields b.
func blendHex(a, b lipgloss.Color, t float64) string {
	if t < 0 {
		t = 0
	}
	if t > 1 {
	...
	ar, ag, ab, okA := parseRGB(string(a))
	br, bg, bb, okB := parseRGB(string(b))
	if !okA || !okB {
		return string(a)
	}
