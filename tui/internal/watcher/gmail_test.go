...
func TestGmailAdapter_RenewalLoop_FiresOnceBeforeExpiry(t *testing.T) {
	...
	initialExpiry := fakeNow.Add(1 * time.Hour)
	a.watchExpiry = initialExpiry
	a.lastHealthErr = fmt.Errorf("renewal has not completed")
	...
	// Poll until registerWatch and the renewal loop's success branch have both
	// completed. WatchCalls increments when the HTTP handler starts, which is
	// too early to cancel the request under a loaded race run.
	...
	for {
		...
		a.mu.Unlock()
		if fs.WatchCalls() >= 1 && !expiry.Equal(initialExpiry) && healthErr == nil {
			break
		}
		if time.Now().After(deadline) {
			cancel()
			<-done
			t.Fatalf("renewal never completed; watchCalls=%d expiry=%s healthErr=%v",
				fs.WatchCalls(), expiry, healthErr)
		}
