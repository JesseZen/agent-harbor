...
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/agent-harbor-coreclient-check.XXXXXX")

trash_temp_root() {
	[ -d "$TEMP_ROOT" ] || return 0
	TRASH_ROOT=${TMPDIR:-/tmp}/agent-harbor-trash
	mkdir -p "$TRASH_ROOT"
	mv -- "$TEMP_ROOT" "$TRASH_ROOT/coreclient-check.$$"
}
...
go run github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.7.2 \
	...
	-o "$TEMP_ROOT/admin.gen.go" \
	"$TEMP_ROOT/admin.bundle.yaml"
gofmt -w "$TEMP_ROOT/admin.gen.go"
diff -u internal/coreclient/generated/admin.gen.go "$TEMP_ROOT/admin.gen.go"
