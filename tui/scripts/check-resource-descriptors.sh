...
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/agent-harbor-descriptors-check.XXXXXX")

trash_temp_root() {
	[ -d "$TEMP_ROOT" ] || return 0
	TRASH_ROOT=${TMPDIR:-/tmp}/agent-harbor-trash
	mkdir -p "$TRASH_ROOT"
	mv -- "$TEMP_ROOT" "$TRASH_ROOT/descriptors-check.$$"
}
...
go run ./internal/resourcepage/gendesc \
	...
	-output "$TEMP_ROOT/descriptors.go"
gofmt -w "$TEMP_ROOT/descriptors.go"
diff -u internal/resourcepage/generated/descriptors.go "$TEMP_ROOT/descriptors.go"
